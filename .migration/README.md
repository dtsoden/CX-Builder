...
The Docker Hub image `dsoden/cx-builder:latest` is multi-architecture
(`linux/amd64` + `linux/arm64`). See the "Pushing to Docker Hub" section of the
root README for the exact commands. Two things that are easy to get wrong:

- It builds from the root `Dockerfile`, not `docker/Dockerfile.local`. The README
  previously documented the wrong one.
- It needs the `docker-container` buildx driver. The default driver cannot emit a
  manifest list, so `--platform linux/amd64,linux/arm64` silently will not work.

arm64 is built under QEMU emulation on x86 hosts and runs 10-15x slower than amd64
(~11 minutes vs ~50 seconds for the build stage).

## Verified on 3.1.4

The amd64 image was booted end to end: migrations applied, `/api/v1/ping` returned
200, `/api/v1/version` reported 3.1.4, the marketplace folder derivation resolved
correctly (Chat 24 chatflow, Agents-V1 14 v1, Agents-V2 13 v2, Tools 13 tool), and
the UI served with CX-Builder branding. The arm64 image was built but not booted.
