...
/**
 * Determines the display type and canvas type from a folder name.
 * Convention:
 * - "tools" (case insensitive) → type: "Tools", canvasType: "tool"
 * - Ends with "-V1" (case insensitive) → canvasType: "v1", type: folder name without suffix
 * - Ends with "-V2" (case insensitive) → canvasType: "v2", type: folder name without suffix
 * - Otherwise → canvasType: "chatflow", type: folder name as-is
 */
const getFolderTypeInfo = (folderName: string): { displayType: string; canvasType: string } => {
    const lowerName = folderName.toLowerCase()

    // Check for Tools folder (case insensitive)
    if (lowerName === 'tools') {
        return { displayType: 'Tools', canvasType: 'tool' }
    }

    // Check for -V1 suffix (case insensitive)
    if (lowerName.endsWith('-v1')) {
        return { displayType: folderName.slice(0, -3), canvasType: 'v1' }
    }

    // Check for -V2 suffix (case insensitive)
    if (lowerName.endsWith('-v2')) {
        return { displayType: folderName.slice(0, -3), canvasType: 'v2' }
    }

    // Default: chatflow canvas, folder name as type
    return { displayType: folderName, canvasType: 'chatflow' }
}

// Get all templates for marketplaces (dynamically scans folders)
const getAllTemplates = async () => {
    try {
        const marketplacesRoot = path.join(__dirname, '..', '..', '..', 'marketplaces')
        let templates: any[] = []

        // Get all subdirectories in marketplaces folder
        const folders = fs.readdirSync(marketplacesRoot, { withFileTypes: true })
            .filter(dirent => dirent.isDirectory())
            .map(dirent => dirent.name)

        for (const folderName of folders) {
            const folderPath = path.join(marketplacesRoot, folderName)
            const { displayType, canvasType } = getFolderTypeInfo(folderName)

            // Get all JSON files in this folder
            const jsonsInDir = fs.readdirSync(folderPath).filter((file) => path.extname(file) === '.json')
            ...
            for (const file of jsonsInDir) {
                ...
                if (canvasType === 'tool') {
                    // Tool templates have special structure
                    const template = {
                        ...fileDataObj,
                        id: uuidv4(),
                        type: displayType,
                    ...
                    const template = {
                        ...
                        framework: fileDataObj?.framework,
                        usecases: fileDataObj?.usecases,
                        categories: getCategories(fileDataObj as ITemplate),
                        type: displayType,
                        canvasType: canvasType,
        ...
        const sortedTemplates = templates.sort((a, b) => {
            // Prioritize V2 (Hero) templates first
            if (a.canvasType === 'v2' && b.canvasType !== 'v2') {
                return -1
            }
            if (b.canvasType === 'v2' && a.canvasType !== 'v2') {
                return 1
            }
            // Then V1 templates
            if (a.canvasType === 'v1' && b.canvasType !== 'v1') {
                return -1
            }
            if (b.canvasType === 'v1' && a.canvasType !== 'v1') {
                return 1
            }
            // Put Tool templates last
            if (a.canvasType === 'tool' && b.canvasType !== 'tool') {
                return 1
            }
            if (b.canvasType === 'tool' && a.canvasType !== 'tool') {
                return -1
            }
            // For same canvas types, sort alphabetically by templateName
