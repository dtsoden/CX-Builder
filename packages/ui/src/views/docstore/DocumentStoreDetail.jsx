...
import EmptyStateImage from '@/ui-component/EmptyStateImage'
...
const DocumentStoreDetails = () => {
    ...
    const onDocStoreDelete = async (type, file) => {
        ...
        if (type === 'STORE') {
            ...
            try {
                ...
                setError(error)
                enqueueSnackbar({
                    message: `Failed to delete ${docstoreLabel}: ${
    ...
    const onLoaderDelete = (file, vectorStoreConfig, recordManagerConfig) => {
        ...
        let description = `Delete "${displayName}"? This will delete all the associated document chunks from the ${docstoreLabel}.`
        ...
        if (
            ...
            description = `Delete "${displayName}"? This will delete all the associated document chunks from the ${docstoreLabel} and remove the actual data from the vector store database.`
        }
    ...
    const onStoreDelete = (vectorStoreConfig, recordManagerConfig) => {
        let description = `Delete Store ${getSpecificDocumentStore.data?.name}? This will delete all the associated loaders and document chunks from the ${docstoreLabel}.`
        ...
        if (
            ...
            description = `Delete Store ${getSpecificDocumentStore.data?.name}? This will delete all the associated loaders and document chunks from the ${docstoreLabel}, and remove the actual data from the vector store database.`
        }
    ...
    const onStoreRefresh = async (storeId) => {
        ...
        if (isConfirmed) {
            ...
            try {
                ...
                if (resp.data) {
                    enqueueSnackbar({
                        message: `${docstoreLabel} refresh successfully!`,
                        options: {
                            key: new Date().getTime() + Math.random(),
                            variant: 'success',
                            action: (key) => (
                ...
                setBackdropLoading(false)
                enqueueSnackbar({
                    message: `Failed to refresh ${docstoreLabel}: ${
                        typeof error.response.data === 'object' ? error.response.data.message : error.response.data
                    }`,
                    options: {
                        key: new Date().getTime() + Math.random(),
