...
const Marketplace = () => {
    ...
    const goToCanvas = (selectedChatflow) => {
        // Route based on canvasType: v2 (Hero), v1 (legacy Hero), chatflow (default)
        if (selectedChatflow.canvasType === 'v2') {
            navigate(`/v2/marketplace/${selectedChatflow.id}`, { state: selectedChatflow })
        } else if (selectedChatflow.canvasType === 'v1') {
            navigate(`/v1/marketplace/${selectedChatflow.id}`, { state: selectedChatflow })
        } else {
        ...
        if (getAllTemplatesMarketplacesApi.data) {
            try {
                ...
                for (let i = 0; i < flows.length; i += 1) {
                    // Extract type from all templates (including Tools)
                    if (flows[i].type) {
                        extractedTypes.push(flows[i].type)
                    }
                    if (flows[i].flowData) {
                    ...
                    } else if (flows[i].usecases) {
                        // Handle Tools which don't have flowData but may have usecases
                        usecases.push(...(flows[i].usecases || []))
                    }
                }
                setImages(images)
                ...
                setTypes(Array.from(new Set(extractedTypes)).sort())
            } catch (e) {
                console.error(e)
            }
        }
                                                ...
                                                {getAllTemplatesMarketplacesApi.data
                                                    ...
                                                    .map((data, index) => (
                                                        <Box key={index}>
                                                            ...
                                                            {!data.badge && data.canvasType !== 'tool' && (
                                                                <ItemCard
                                                                    onClick={() => goToCanvas(data)}
                                                                    data={data}
                                                                    images={images[data.id]}
                                    ...
                                    (!getAllTemplatesMarketplacesApi.data || getAllTemplatesMarketplacesApi.data.length === 0) && (
                                        <Stack sx={{ alignItems: 'center', justifyContent: 'center' }} flexDirection='column'>
                                            <Box sx={{ p: 2, height: 'auto' }}>
                                                <EmptyStateImage alt='No Marketplace' />
                                            </Box>
                        ...
                        <Available permission='templates:custom'>
                            <TabPanel value={activeTabValue} index={1}>
                                ...
                                {!isLoading && (!getAllCustomTemplatesApi.data || getAllCustomTemplatesApi.data.length === 0) && (
                                    <Stack sx={{ alignItems: 'center', justifyContent: 'center' }} flexDirection='column'>
                                        <Box sx={{ p: 2, height: 'auto' }}>
                                            <EmptyStateImage alt='No Custom Templates' />
                                        </Box>
