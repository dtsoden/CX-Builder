    ...
    const onGenerate = async () => {
        ...
        try {
            ...
            if (response.data && response.data.nodes && response.data.edges) {
                ...
                enqueueSnackbar({
                    message: response.error || `Failed to generate ${agentflowLabel}`,
                    options: {
            ...
            enqueueSnackbar({
                message: error.response?.data?.message || `Failed to generate ${agentflowLabel}`,
                options: {
                    key: new Date().getTime() + Math.random(),
                    variant: 'error',
                    persist: false,
                ...
                <DialogContent>
                    {loading ? (
                        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column' }}>
                            <img src={generatorGIF} alt={`Generating ${agentflowLabel}`} style={{ maxWidth: '100%', height: 'auto' }} />
                        ...
                        <>
                            ...
                            <Box sx={{ mt: 2 }}>
                                <div style={{ display: 'flex', flexDirection: 'row' }}>
                                    <Typography>
                                        Select model to generate {agentflowLabel}<span style={{ color: 'red' }}>&nbsp;*</span>
                                    </Typography>
                                </div>
                                <Dropdown
